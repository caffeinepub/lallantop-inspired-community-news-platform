...
import { Menu, X, Heart, LayoutDashboard, Mail, Globe, Loader2 } from 'lucide-react';
...
export default function Layout() {
  ...
  // Only show profile setup when we've successfully fetched and confirmed there's no profile.
  // If the query errored (backend trap), don't show the modal — treat the user as a new visitor.
  const showProfileSetup = isAuthenticated && !profileLoading && isFetched && userProfile === null && !profileError;

  // Show backend connection status when authenticated but actor not yet ready
  const showConnecting = isAuthenticated && !actor && actorFetching;
  const showConnectionFailed = isAuthenticated && !actor && !actorFetching;
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      {(showConnecting || showConnectionFailed) && (
        <div
          className={`text-center text-xs py-1.5 px-4 ${
            showConnecting
              ? 'bg-news-blue/20 text-news-blue-light'
        ...
        >
          {showConnecting ? (
            ...
            <span>
              Connection failed. Please refresh the page or try logging out and back in.
            </span>
          )}
        </div>
      )}
        ...
        <div className="max-w-[1400px] mx-auto px-3 py-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            ...
            <div className="md:col-span-2">
              ...
              <p className="text-xs leading-relaxed text-white/50 max-w-xs mb-3">
              ...
              <div className="flex flex-col gap-1.5">
                <a
                  ...
                  className="flex items-center gap-1.5 text-news-blue-light hover:text-white text-xs transition-colors"
                >
                  <Mail size={11} />
          ...
          <div className="border-t border-white/10 pt-4 flex flex-col sm:flex-row items-center justify-between gap-2 text-[11px] text-white/40">
            <span>© {new Date().getFullYear()} Global Nexus |{' '}
              <a
                href="https://globalnexus.co.in"
                target="_blank"
                rel="noopener noreferrer"
                className="text-news-blue-light hover:text-white transition-colors"
              >
                globalnexus.co.in
              </a>
            </span>
